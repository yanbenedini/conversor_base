...
string decimalParaHexadecimal(string numero) {
    ...
    while (numero_int > 0) {
        ...
        if (resto < 10)
            ...
            hexadecimal = char(resto - 10 + 'A') + hexadecimal; //Tranforma o resto em uma letra entre A e F
        numero_int /= 16;
    }
    return hexadecimal;
}
...
int main(){
    ...
    while (valid == false) { //numero.length devolve o numero de caracteres da string (o comprimento dela)
        for (int i = str_numero.length() - 1; i >= 0; i--) { //Testa o cada caracter da string de tras pra frente
            if (!isdigit(str_numero[i]) && str_numero[i] != 'A' && str_numero[i] != 'B' && str_numero[i] != 'C' && str_numero[i] != 'D' && str_numero[i] != 'E' && str_numero[i] != 'F') {
                cout << "Um dos caracteres inseridos nao eh valido para conversao, por favor tente novamente: \n";
                cout << "Qual algarismo voce deseja mudar de base?: \n";
                valid = false;
                cin >> str_numero;
                i = str_numero.length(); // reseta a variavel i para o comprimento da string numero
            } else {
                valid = true;
            }
        }
    ...
    if (base1 == 2) {
        while (v == false){
            for (int i = 0; i < str_numero.length(); i++) {
                if ((str_numero[i] != '0') && (str_numero[i] != '1')) {
                    cout << "Erro: numero invalido, tente novamente: " << endl;
                    cin >> str_numero; 
                    i = 0;
                    v = false;
                } else {
        ...
        while (v == false){
            for (int i = 0; i < str_numero.length(); i++) {
                if (str_numero[i] != '0' && str_numero[i] != '1' && str_numero[i] != '2') {
                    cout << "Erro: numero invalido, tente novamente: " << endl;
                    cin >> str_numero; 
                    i = 0;
                    v = false;
        ...
        while (v == false){
            for (int i = 0; i < str_numero.length(); i++) {
                if (str_numero[i] != '0' && str_numero[i] != '1' && str_numero[i] != '2' && str_numero[i] != '3' && str_numero[i] != '4' && str_numero[i] != '5' && str_numero[i] != '6' && str_numero[i] != '7') {
                    cout << "Erro: numero invalido, tente novamente: " << endl;
                    cin >> str_numero; 
                    i = 0;
                    v = false;
